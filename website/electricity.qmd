---
title: European electricity availability challenge 
---
...
Electricity availability statistics are a key part of the system of energy statistics in the EU and have a long history of data collection.
For this indicator, the benchmark will be the official figures on Electricity availability in each European country.
...
targets::tar_load(predictions_electricity, store = "store_electricity")

sample <- data$ELECTRICITY$data |>
  dplyr::filter((siec %in% "E7000"))

ojs_define(data = sample, pred = predictions_electricity)
```
